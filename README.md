...
1. **ReportGenerator.Core**: ספרייה מרכזית להפקת דוחות
   - מנהל תבניות HTML
   - מעבד תבניות עם פלייסהולדרים
   - ממיר HTML-to-PDF
   - יוצר דוחות Excel
   - מערכת ניהול שגיאות מתקדמת

2. **ReportGenerator.Api**: Web API לשירותי דוחות
   - REST API מלא
   - תמיכה בדוחות PDF ו-Excel
   - Swagger UI מובנה
   - endpoint מיוחד לקבלת raw bytes

3. **RunApiAndSaveReport**: כלי לקוח (EXE)
   - הפעלה מול ה-API
   - שמירה מקומית של קבצי דוחות
   - מצב אינטראקטיבי ו-command line
   - טיפול בשגיאות רשת
